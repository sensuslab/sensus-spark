---
title: 'Task UI'
description: 'Documentation for the Spark Task UI'
---

## Spark Task UI

The Spark Task UI provides a web-based interface for interacting with the Spark agent system. It combines a action feed with an embedded noVNC viewer, allowing you to watch it perform task on the desktop in real-time.

<img src="/static/chat-ui-overview.png" alt="Spark Task Detail" className="w-full max-w-4xl" />

## Accessing the UI

When running the full Spark agent system, the Task UI is available at:
...
The task interface displays different types of messages based on Spark's content block structure:
...
The Spark Task UI is built with:
...
1. Ensure all Spark services are running
...
1. Ensure the Spark container is running
