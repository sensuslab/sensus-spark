...
# Creating and Managing Tasks in Spark

This guide will walk you through everything you need to know about creating tasks that Spark can execute effectively, and managing them through their lifecycle.

## Understanding Tasks

A task is any job you want Spark to complete. Tasks can be:
...
Spark has powerful file handling capabilities that make it perfect for document processing and data analysis tasks.
...
- **Document Processing**: Upload PDFs, spreadsheets, or documents for Spark to analyze
...
<Note>
  **Game Changer**: Spark can read entire files, including PDFs, directly into the LLM context. This means it can process large amounts of data quickly and understand complex documents without manual extraction.
</Note>
...
<Steps>
  ...
  <Step title="Start Task">
    Press the arrow button or hit Enter. Spark will start loading and begin working on your task.
  </Step>
</Steps>
...
(Spark handles all authentication including 2FA automatically via password manager)
```
...
Watch Spark work through the task detail viewer:

- **Green dot**: Task is actively running
- **Status messages**: Current step being executed
- **Desktop view**: See what Spark sees in real-time

#### Chat Messages

Spark provides updates like:
```
...
(Note: Spark handles all authentication including 2FA automatically using the built-in password manager)
...
After each task:
1. Review the approach Spark took
...
<Note>
  **Pro Tip**: Start with simple tasks to understand Spark's capabilities, then gradually increase complexity as you learn what works best.
</Note>