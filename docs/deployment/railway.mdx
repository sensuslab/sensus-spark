---
title: "Deploying Spark on Railway"
description: "Comprehensive guide to deploying the full Spark stack on Railway using the official 1-click template"
---

> **TL;DR –** Click the button below, add your AI API key (Anthropic, OpenAI, or Google), and your personal Spark instance will be live in ~2 minutes.
...
Railway provides a zero-ops PaaS experience with private networking and per-service logs that perfectly fits Spark’s multi-container architecture. The official template wires every service together using the latest container images pushed to the `edge` branch.
