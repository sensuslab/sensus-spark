...
description: "Use any LLM provider with Spark through LiteLLM proxy"
---

# Connect Any LLM to Spark with LiteLLM

LiteLLM acts as a unified proxy that lets you use 100+ LLM providers with Spark - including Azure OpenAI, AWS Bedrock, Anthropic, Hugging Face, Ollama, and more. This guide shows you how to set up LiteLLM with Spark.
...
## Quick Start with Spark's Built-in LiteLLM Proxy

Spark includes a pre-configured LiteLLM proxy service that makes it easy to use any LLM provider. Here's how to set it up:
...
<Steps>
  <Step title="Use Docker Compose with Proxy">
    ...
    # Start Spark with LiteLLM proxy
  ...
  <Step title="Verify Models are Available">
    The Spark agent automatically queries the proxy for available models:
    
    ```bash
    # Check available models through Spark API
...
```mermaid
graph LR
    A[Spark UI] -->|Select Model| B[Spark Agent]
...
# Use "smart-model" in Spark config
```
...
<AccordionGroup>
  ...
  <Accordion title="Authentication errors">
    Verify master key in both LiteLLM and Spark:
    ```bash
    # Test LiteLLM
...
<Note>
  **Pro tip:** Start with a single provider, then add more as needed. LiteLLM makes it easy to switch or combine models without changing Spark configuration.
</Note>