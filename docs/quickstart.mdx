...
Spark can be deployed in several ways depending on your needs:
...
<Steps>
  <Step title="Visit the Template">
    Click the Deploy Now button in the Spark template on Railway.
  </Step>
...
- **Spark Desktop**: Containerized Linux environment
...
Navigate to [http://localhost:9992](http://localhost:9992) to access the Spark UI.

**Two ways to interact:**
1. **Tasks**: Enter task descriptions to have Spark work autonomously
...
<Note>
  **🔐 Password Manager Support**: Spark can handle authentication automatically when you install a password manager extension. See our [password management guide](/guides/password-management) for setup instructions.
</Note>
...
Now let's see Spark in action! Try these example tasks:
...
curl -X POST http://localhost:9990/computer-use \
  -H "Content-Type: application/json" \
  -d '{"action": "type_text", "text": "Hello, Spark!"}'
```
